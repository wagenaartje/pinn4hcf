...
def loss_inlet (x_bc, u_bc):
    ...
    loss = (disc[:,4] * (r - u_bc[0])).mean() + \
           (disc[:,5] * (p - u_bc[1])).mean() + \
           (disc[:,6] * (u - u_bc[2])).mean() + \
           (disc[:,7] * (v - u_bc[3])).mean()
    
    mse = ((r - u_bc[0])**2).mean().item() + \
          ((p - u_bc[1])**2).mean().item() + \
          ((u - u_bc[2])**2).mean().item() + \
          ((v - u_bc[3])**2).mean().item()
...
if __name__ == '__main__':
    ...
    # Prepare logging
    checkpoint_file = open(f'./oblique/cpinn_phi_viscosity.p', 'ab')
