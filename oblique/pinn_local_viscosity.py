...
def evaluate ():
    ...
    loss = loss_r + 1 * loss_bc + 1e-1 * loss_v
    loss.backward()
...
if __name__ == '__main__':
    ...
    # Prepare logging
    checkpoint_file = open(f'./oblique/pinn_local_viscosity.p', 'ab')
    ...
    for i in range(5000):
        ...
        # Print status
        print(f'Epoch: {epoch:4d} - Time: {time.time() - epoch_start_time:.2f} - MSE_r: {mse_pde:.2e} - MSE_v: {mse_v:.2e} - L2_r: {L2_r:.2e} - L2_p: {L2_p:.2e} - L2_u: {L2_u:.2e} - L2_v: {L2_v:.2e}')
        
        epoch += 1
 
        # Termination conditions
        if torch.isnan(loss) or (epoch - lowest_loss_epoch > 10):
