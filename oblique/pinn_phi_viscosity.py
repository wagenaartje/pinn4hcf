...
def reference (device='cpu'):
    ...
    x_test = torch.tensor(data['x_test'], device=device)
    y_test = torch.tensor(data['y_test'], device=device)
    u_test = torch.tensor(data['u_test'], device=device)
...
def loss_pde (coords):
    ...
    state = generator(coords)
    r1, r2, r3, r4 = pde(state, coords, 1e-3)
...
if __name__ == '__main__':
    ...
    # Prepare logging
    checkpoint_file = open(f'./oblique/pinn_phi_viscosity.p', 'ab')
