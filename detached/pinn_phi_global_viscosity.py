...
def reference (device='cpu'):
    data = np.load('./detached/reference/reference.npz')
...
def loss_pde (coords):
    ...
    state = generator(coords)
    r1, r2, r3, r4 = pde(state, coords,torch.abs(viscosity))
...
def loss_inlet (x_bc):
    ...
    #print(torch.mean(U).item(), torch.mean(u).item(), torch.mean(v).item(), u_inf, v_inf)
...
def evaluate ():
    ...
    loss = loss_r + loss_bc + 1e-1 * loss_v
    loss.backward()
...
if __name__ == '__main__':
    ...
    viscosity = torch.nn.Parameter(torch.ones([1],dtype=torch.float32, device=device), requires_grad=True)
    generator = Generator().to(device)
    params = list(generator.parameters())
    params.extend([viscosity])
    optimizer = torch.optim.LBFGS(params, lr=0.1)

    # Prepare logging
    checkpoint_file = open(f'./detached/pinn_phi_global_viscosity.p', 'ab')
    ...
    for i in range(5000):
        ...
        # Print status
        print(f'Epoch: {epoch:4d} - Time: {time.time() - epoch_start_time:.2f} - MSE_r: {mse_pde:.2e} - v: {viscosity.abs().item():.2e} - L2_r: {L2_r:.2e} - L2_p: {L2_p:.2e} - L2_u: {L2_u:.2e} - L2_v: {L2_v:.2e}')
