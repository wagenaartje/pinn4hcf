...
class Generator (torch.nn.Module):
    ...
    def forward (self, a: tuple) -> torch.Tensor:
        ...
        res[:,2] = torch.exp(res[:,2])

        res[:,3] = torch.tanh(res[:,3]) * torch.pi # NOTE! only forward flow allowed now
...
if __name__ == '__main__':
    ...
    # Prepare logging
    checkpoint_file = open(f'./detached/cpinn_phi_viscosity_neumann.p', 'ab')
    ...
    while True:
        ...
        # Print status
        print(f'Epoch: {epoch:4d} - Time: {time.time() - epoch_start_time:.2f} - MSE_r: {mse_pde:.2e} - MSE_bc: {mse_bc:.2e} - L2_r: {L2_r:.2e} - L2_p: {L2_p:.2e} - L2_u: {L2_u:.2e} - L2_v: {L2_v:.2e}')
